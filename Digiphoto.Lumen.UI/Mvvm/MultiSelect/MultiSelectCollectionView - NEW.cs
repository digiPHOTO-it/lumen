	...
	public class MultiSelectCollectionView<T> : ListCollectionView, IMultiSelectCollectionView {
		...
		void SetSelection( Selector selector ) {
			...
			if( multiSelector != null ) {
				...
				// Qui concettualmente devo riversare this.selecteditems in listbox.selecteditems
				// per essere piu efficiente, gestisco dei casi particolari per essere veloce.
				// concettualmente devo fare questo:
		...
		private void addUiControl( T elem ) {

			foreach( Control control in controls )
				addSelection( (Selector)control, elem );
		}

		void addSelection( Selector selector, T elem ) {
			...
			if( multiSelector != null ) {
				...
				if( ! multiSelector.SelectedItems.Contains( elem ) )
					multiSelector.SelectedItems.Add( elem );

			} else if( listBox != null ) {

				if( ! listBox.SelectedItems.Contains( elem ) )
					listBox.SelectedItems.Add( elem );
			}
		}

		private void removeUiControl( T elem ) {

			foreach( Control control in controls )
				removeSelection( (Selector)control, elem );
		}

		void removeSelection( Selector selector, T elem ) {
			...
			if( multiSelector != null ) {
				...
				if( multiSelector.SelectedItems.Contains( elem ) )
					multiSelector.SelectedItems.Remove( elem );

			} else if( listBox != null ) {

				if( listBox.SelectedItems.Contains( elem ) )
					listBox.SelectedItems.Remove( elem );
			}
		}

		public bool deseleziona( T elem ) {

			bool ret = false;

			if( SelectedItems.Contains( elem ) ) {

				bool save = ignoreSelectionChanged;
                this.ignoreSelectionChanged = true;

				try {

					// Tolgo l'elemento dalla mia collezione...
					ret = this.SelectedItems.Remove( elem );

					// Invece che ridisegnare tutto, lavoro puntuale sull'elemento
					// UpdateUiControls();
					// ... e poi dalla collezione visuale
					removeUiControl( elem );
					

				} finally {
					this.ignoreSelectionChanged = save;
				}
