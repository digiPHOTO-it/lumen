...
Public MustInherit Class BaseConfiguration
    ...
    Private Shared Function getMachineCode() As String
        ...
        If collectedInfo = "" Or collectedInfo = "00" Or collectedInfo.Length <= 3 Then
    ...
    ' </returns>
    Private Shared Function getHddSerialNumber() As String


        ' --- Win32 Disk 
        Dim searcher As New ManagementObjectSearcher("\root\cimv2", "select * from Win32_DiskPartition WHERE BootPartition=True")

        Dim diskIndex As UInteger = 999
        For Each partition As ManagementObject In searcher.[Get]()
            diskIndex = CUInt(partition("Index"))
            Exit For
        Next

        ' I haven't found the bootable partition. Fail.
        If diskIndex = 999 Then Return String.Empty
        ...
        If String.IsNullOrEmpty(deviceName.Trim()) Then Return String.Empty

        ' -- Some problems in query parsing with backslash. Using like operator
        If deviceName.StartsWith("\\.\") Then
